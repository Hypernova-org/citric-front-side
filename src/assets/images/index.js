export { default as AboutImg1 } from "./AboutImg.png";
export { default as AboutImg2 } from "./AboutImg2.png";
export { default as AboutImg3 } from "./AboutImg3.png";
export { default as AboutImg4 } from "./AboutImg4.png";
export { default as HeroImgLaptop } from "./Citric@2x.webp";
export { default as HeroImgTablet } from "./Citric@2x.webp";
export { default as HeroImgMobile } from "./Citric@2x.webp";
export { default as WhiteLogo } from "./logo(white).png";
export { default as AboutSection1 } from "./About-section-img.webp";
export { default as AboutSection2 } from "./About-section-img2.webp";
